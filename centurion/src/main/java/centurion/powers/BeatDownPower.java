...
public class BeatDownPower extends AbstractDefaultPower {

    public static final String POWER_ID = centurion.CenturionMod.makeID(BeatDownPower.class.getSimpleName());

    public BeatDownPower(AbstractCreature owner, int amount) {
    ...
    private class MyCallback implements Consumer<List<AbstractCard>> {
        ...
        public void accept(List<AbstractCard> cards) {
            if (cards.size() > 0)
            {
                ...
                AbstractCard reward = new UltimateStrike();
